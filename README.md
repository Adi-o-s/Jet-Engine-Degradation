...
This project utilises MLflow for experiment tracking within the `pipelines/rnn_pipeline.py` script. To run experiments and view results:

1. **Set Up MLflow**:
   - Install MLflow with `pip install mlflow`.

2. **Run the MLflow-Enabled Script**:
   - Execute the pipeline script to kick off experiments:
     ```bash
     python pipelines/rnn_pipeline.py
     ```
   - Runs are logged to `./mlruns` by default. Override the location with the `MLFLOW_DIR` environment variable.
